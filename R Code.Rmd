...
imdb = read.csv("IMDB Fall 2023.csv")
attach(imdb)
...
# Assuming 'imdb' is your dataframe and 'imdb_score' is the column with the IMDb scores
...
```{r}
# Assuming your dataframe is called 'imdb'
...
Drawing boxplots for all predictors
...
```{r}
#Checking if there is/are any combination(s) with value > 0.80 for coorelation
library(reshape2)
...
#We are getting zero rows so no issues for colinearity
...
#residualPlots(reg)
#Consider removing distributors, keywords(must)
...
```{r}
reg3 = lm(imdb_score~.+movie_budget*actor1_star_meter,data = imdb)
summary(reg3)
```
...
fit=lm(imdb_score~.+genre_Romance*movie_budget+maturity_ratingPG*nb_faces-genre_Romance-maturity_ratingPG+genre_Crime*movie_meter_IMDBpro-genre_Crime+movie_meter_IMDBpro*genre_Adventure+movie_meter_IMDBpro*famous_production_company+poly(movie_budget,2)+poly(duration,1,raw=TRUE)+poly(nb_news_articles,2)+poly(movie_meter_IMDBpro,3)-movie_meter_IMDBpro-movie_budget-nb_news_articles-duration+genre_Thriller*cinematographer_less_experienced+nb_faces*release_monthApr+nb_faces*director_very_prolific+actor1_star_meter*genre_Fantasy-release_monthFeb-actor1_star_meter+genre_Horror*release_monthJun-release_monthJun-release_monthNov-release_monthAug-genre_History-release_monthDec-actor3_star_meter-cinematographer_experienced-cinematographer_less_experienced-director_very_prolific-genre_Sci_Fi+genre_Drama*genre_Thriller-release_monthJul-genre_Mystery-genre_Thriller-genre_Biography-aspect_ratio_2_35-actor2_star_meter+major_distributor*release_monthSep-major_distributor-genre_Fantasy-release_monthApr-genre_Sport-genre_War,data = train_set)
...
```{r}
library(boot)
fit1=glm(imdb_score~.+genre_Romance*movie_budget+maturity_ratingPG*nb_faces-genre_Romance-maturity_ratingPG+genre_Crime*movie_meter_IMDBpro-genre_Crime+movie_meter_IMDBpro*genre_Adventure+movie_meter_IMDBpro*famous_production_company+poly(movie_budget,2)+poly(duration,1,raw=TRUE)+poly(nb_news_articles,2)+poly(movie_meter_IMDBpro,3)-movie_meter_IMDBpro-movie_budget-nb_news_articles-duration+genre_Thriller*cinematographer_less_experienced+nb_faces*release_monthApr+nb_faces*director_very_prolific+actor1_star_meter*genre_Fantasy-release_monthFeb-actor1_star_meter+genre_Horror*release_monthJun-release_monthJun-release_monthNov-release_monthAug-genre_History-release_monthDec-actor3_star_meter-cinematographer_experienced-cinematographer_less_experienced-director_very_prolific-genre_Sci_Fi+genre_Drama*genre_Thriller-release_monthJul-genre_Mystery-genre_Thriller-genre_Biography-aspect_ratio_2_35-actor2_star_meter+major_distributor*release_monthSep-major_distributor-genre_Fantasy-release_monthApr-genre_Sport-genre_War-genre_Musical-release_monthOct-release_monthMar-release_monthJan-num_movies-maturity_ratingR,data = imdb)
...
tests = read.csv("test data Fall 2023.csv")
attach(tests)
#Removing genres dummy as they are not covering all the genres
tests = tests[,-(27:39)]
...
# Assuming your data is in a dataframe called 'tests'
...
numeric_columns <- tests %>%
  select_if(~ is.numeric(.) && !identical(., tests$tests_score)) %>%
...
#tests = tests[-c(40,87,128,823,1331,1360,1498,1692),]
#tests = tests[,-c(31)]
```
